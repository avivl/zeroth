...
func (s *Store) AppendAudit(ctx context.Context, r store.AuditRecord) (store.AuditRecord, error) {
	...
	if r.ID.IsZero() || r.Action == "" || r.ResourceType == "" || r.ResourceID == "" || r.Signature == "" {
	...
	_, err := s.db.ExecContext(ctx, `
		INSERT INTO audit_records (id, action, resource_type, resource_id, actor, signature, created_at_unix_nano)
		VALUES (?, ?, ?, ?, ?, ?, ?)`,
		r.ID.String(), r.Action, r.ResourceType, r.ResourceID, r.Actor, r.Signature, r.CreatedAt.UnixNano(),
	)
	if err != nil {
...
func (s *Store) GetAudit(ctx context.Context, id store.AuditID) (store.AuditRecord, error) {
	...
	err := s.db.QueryRowContext(ctx, `
		SELECT id, action, resource_type, resource_id, actor, signature, created_at_unix_nano
		FROM audit_records WHERE id = ?`, id.String(),
	).Scan(&rawID, &r.Action, &r.ResourceType, &r.ResourceID, &r.Actor, &r.Signature, &created)
	if err != nil {
		return store.AuditRecord{}, wrap("get audit", err)
	}
	aid, err := store.ParseAuditID(rawID)
	if err != nil {
		return store.AuditRecord{}, wrap("get audit", err)
	}
	r.ID = aid
...
func (s *Store) ListAudit(ctx context.Context, q store.AuditQuery) (store.Page[store.AuditRecord], error) {
	...
	query := `SELECT id, action, resource_type, resource_id, actor, signature, created_at_unix_nano FROM audit_records`
	if len(conds) > 0 {
		query += ` WHERE ` + strings.Join(conds, " AND ")
	}
	query += ` ORDER BY created_at_unix_nano DESC, id DESC LIMIT ?`
	...
	for rows.Next() {
		...
		var rawID string
		var created int64
		if err := rows.Scan(&rawID, &r.Action, &r.ResourceType, &r.ResourceID, &r.Actor, &r.Signature, &created); err != nil {
			return store.Page[store.AuditRecord]{}, wrap("list audit", err)
		}
		aid, err := store.ParseAuditID(rawID)
		if err != nil {
