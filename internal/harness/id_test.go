...
func TestNewID(t *testing.T) {
	...
	parsed, err := harness.ParseID(a.String())
	if err != nil {
		t.Fatal(err)
	}
	if parsed != a {
...
func TestIDIsNotSessionOrSandboxID(t *testing.T) {
	t.Parallel()
	sid, err := session.ParseID("same-raw")
	if err != nil {
		t.Fatal(err)
	}
	bid, err := sandbox.ParseID("same-raw")
	if err != nil {
		t.Fatal(err)
	}
	hid, err := harness.ParseID("same-raw")
	if err != nil {
		t.Fatal(err)
	}
	// Same raw string, distinct types. Mixing them is a compile error
	// (hid = sid does not compile). This test locks the helpers.
	if sid.String() != bid.String() || bid.String() != hid.String() {
