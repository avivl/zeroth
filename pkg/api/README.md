...
| --- | --- | --- |
| Runs | `POST/GET /runs`, `GET /runs/{id}`, `GET /runs/{id}/events`, `POST /runs/{id}/steer`, `POST /runs/{id}/background`, `POST /runs/{id}/foreground`, `POST /runs/{id}/stop`, `POST /runs/{id}/retract`, `POST /runs/{id}/checkpoints` | Live tail is the WebSocket on `GET /runs/{id}/events`. HTTP without Upgrade returns the same replay window for the CLI. Stop checkpoints, then marks the run stopped so restore can fork a continuation. Retract closes a bad already-produced PR and records why on the tracker issue. |
