...
| `cmd/zeroth` | CLI and headless entry point (`run`, `attach`, `bg`, `runs`, `verify`, `reject`) | A second copy of the kernel |
...
`task ci` is a local stand-in if [Task](https://taskfile.dev) is installed. It runs the generated-API freshness check, lint, `go test -race ./...`, conformance, secretscan, the web tests and build, and `go build ./...`. `task --list` shows the other targets (`up`, `lint`, `conformance`, `generate`, `generate:check`, `web`, `secretscan`). GitHub Actions CI lives in `.github/workflows/ci.yml`. The Go job writes `coverage.out` and runs [octocov](https://github.com/k1LoW/octocov): it comments the current percentage, the diff versus main, and the code-to-test ratio on PRs, fails the job if coverage drops (`coverage.acceptable: diff >= 0%` in `.octocov.yml`, not an absolute floor), and on merge to main commits `docs/coverage.svg`. Path filtering skips Go on `web/`-only PRs and web on `internal/`-only PRs. `pkg/api/` changes run everything. CI fails if `pkg/api/gen` is stale relative to `openapi.yaml`.
...
- **Cobra** owns both command trees. `cmd/zeroth` has `version`, `run`, `attach`, `bg`, `runs`, `verify`, and `reject`. `cmd/zerothd` is a Cobra root whose flags are the daemon's startup surface.
