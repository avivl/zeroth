...
`task ci` is a local stand-in if [Task](https://taskfile.dev) is installed. It runs the generated-API freshness check, lint, `go test -race ./...`, conformance, secretscan, the web tests and build, and `go build ./...`. `task --list` shows the other targets (`up`, `lint`, `conformance`, `generate`, `generate:check`, `web`, `secretscan`). GitHub Actions CI lives in `.github/workflows/ci.yml`. Path filtering skips Go on `web/`-only PRs and web on `internal/`-only PRs. `pkg/api/` changes run everything. CI fails if `pkg/api/gen` is stale relative to `openapi.yaml`.
